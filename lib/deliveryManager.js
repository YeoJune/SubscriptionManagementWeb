        ...
        (err, result) => {
          ...
          db.run('BEGIN TRANSACTION', (err) => {
            ...
            deliveryDates.forEach((date, index) => {
              ...
              db.run(
                `INSERT INTO delivery_list (user_id, status, date, product_id, special_request, delivery_sequence) VALUES (?, ?, ?, ?, ?, ?)`,
                [
                  user_id,
                  'pending',
                  date,
    ...
    db.serialize(() => {
      ...
      db.run(
        ...
        function (err) {
          ...
          const insertQuery = `
            INSERT INTO delivery_list (user_id, status, date, product_id, special_request, delivery_sequence)
            VALUES (?, 'pending', ?, ?, ?, ?)
          `;
          ...
          deliveryDates.forEach((date, index) => {
            ...
            db.run(
              insertQuery,
              [userId, date, productId, specialRequest, sequence],
              function (err) {
                if (err) {
                  hasError = true;
                  db.run('ROLLBACK');
