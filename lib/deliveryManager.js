        ...
        (err, result) => {
          ...
          db.run('BEGIN TRANSACTION', (err) => {
            ...
            deliveryDates.forEach((date) => {
              db.run(
                `INSERT INTO delivery_list (user_id, status, date, product_id, special_request) VALUES (?, ?, ?, ?, ?)`,
                [user_id, 'pending', date, product_id, specialRequest],
                function (err) {
                  if (hasError) return;
...
const addDeliveryCount = (userId, productId, count) => {
  return new Promise((resolve, reject) => {
    db.run(
      `INSERT INTO user_product_delivery (user_id, product_id, remaining_count)
       VALUES (?, ?, ?)
    ...
    db.serialize(() => {
      ...
      db.run(
        ...
        function (err) {
          ...
          // 2단계: 배송 스케줄 생성 (specialRequest 포함)
          const insertQuery = `
            INSERT INTO delivery_list (user_id, status, date, product_id, special_request)
            VALUES (?, 'pending', ?, ?, ?)
          `;
          ...
          deliveryDates.forEach((date) => {
            ...
            db.run(
              insertQuery,
              [userId, date, productId, specialRequest],
              function (err) {
                if (err) {
                  hasError = true;
                  db.run('ROLLBACK');
...
async function cancelPaymentDeliveries(user_id, product_id, purchased_count) {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      // 해당 상품의 pending 배송 개수 확인
      db.get(
        `SELECT COUNT(*) as pending_count FROM delivery_list 
