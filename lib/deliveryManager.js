            ...
            db.run(
              ...
              function (err) {
                ...
                function finishUpdate() {
                  // 상태가 'complete'로 변경된 경우 배송 안내 알림톡 발송
                  if (status === 'complete') {
                    db.get(
                      `SELECT u.phone_number, u.name AS user_name, p.name AS product_name,
                              upd.remaining_count
                       FROM users u, delivery_list d, product p, user_product_delivery upd
                       WHERE d.id = ? AND u.id = d.user_id AND p.id = d.product_id 
                       AND upd.user_id = u.id AND upd.product_id = p.id`,
                      [delivery_id],
                      (err, result) => {
                        if (err) {
                          console.error(
                            '배송 안내 알림톡 발송 정보 조회 오류:',
                            err
                          );
                          // 알림톡 발송 실패해도 계속 진행
                        }
    ...
    try {
      ...
      db.all(
        ...
        (err, results) => {
          ...
          // 2가지 알림톡만 사용하므로 잔여 횟수 알림 발송 기능 제거
          // 필요시 관리자가 수동으로 확인 가능
